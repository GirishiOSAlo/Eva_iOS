...
//            filterButtons[1].isHidden = true
    ...
    @objc func addFriend(_ Sender: UIButton) {
        ...
        addConnection(receiverId: id) {
            self.showToast(message: "Connection Added!!")
        }
    }
}
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .pending:
            if requestType == .received {
                ...
                print("Tapped Sent Requests in Pending Tab")
            }
        case .followers:
            let vc = StoryboardRouter.othersProfileVC()
            vc.profileID = id
...
//    }
                ...
                if !(newsRoot.error) {
                    if newsRoot.error {
                        ...
                        if self.requestType == .sent {
                            self.fetchSuggestedConnectionData()
                        }
                        self.stopAPICall = true
                    }
    ...
    func SearchBlockConnection() {
        ...
        var endpoint = EndPoints.blockConnection + "?first_name=\(searchField.text!)"
...
//        let decline: Bool!
//        if inSearchMode {
//            let filteredConnection = filteredConnections[sender.tag]
//            if filteredConnection.isConnected == .pending && filteredConnection.isReceiver == true {
//
//                decline = sender.titleLabel!.text == "Accept" ? false : true
//                updateConnection(id: filteredConnection.connectionID!, index: sender.tag, declined: decline)
//            }
//        } else {
//            let connection = connections[sender.tag]
//            if connection.isConnected == .pending && connection.isReceiver == true {
//                decline = sender.titleLabel!.text == "Accept" ? false : true
//                updateConnection(id: connection.connectionID!, index: sender.tag, declined: decline)
//            }
//        }
...
//        chatVC.conversation = conversation
    ...
    func didViewProfile(connection: UserConnection) {
        if connectionType == .followers {
            ...
            ProfileManager.shared.blockConnection(id: connection.id, isBlock: false) { [weak self] (title, message) in
                guard let self = self else { return }

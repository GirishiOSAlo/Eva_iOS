...
// MARK: - Welcome
struct EditJobDetailsDataModel: Codable {
    let error: Bool?
    let message: String?
    let data: [EditJobDetailsData]?
}
// MARK: - Datum
struct EditJobDetailsData: Codable {
    //let comments: [JSONAny]?
    let id: Int?
    let userID: Int?
    let user: EditJobDetailsUser?
    let jobTitle: String?
    let sectorID, jobNature: Int?
    let jobtype, companyName, companylogo, userimage: String?
    let currencyID: Int?
...
// MARK: - User
struct EditJobDetailsUser: Codable {
    let id: Int?
    let firstName, lastName, email, isConnected: String?
    let isReceiver: Bool?
    let connectionID, bioData, uniqueCode, dateOfBirth: String?
    let status: String?
    let userImage: String?
    let createdByID: String?
    let isLinkedin: Int?
